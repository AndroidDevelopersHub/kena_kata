...
async function list(req ,res ){
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM products WHERE CONCAT(username, email,phone_number) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
    ...
    if (req.params.id){
        db.query("SELECT * FROM `products` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {
                ...
                db.query("UPDATE products SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
