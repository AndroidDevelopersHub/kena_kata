...
module.exports = function (router) {
    ...
    router.get('/checkout/:id', details);
    router.delete('/checkout/:id', _delete);
}
...
function add(req, res){
    ...
    db.query("INSERT INTO checkout SET ?", req.body , (err, result) => {
        if (!err) {
...
async function list(req ,res ){
    ...
    db.query("SELECT COUNT(*) AS total FROM checkout", (err, result) => {
        if (!err) {
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM checkout WHERE CONCAT(username, email,phone_number) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
        ...
        db.query("SELECT * FROM checkout LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
            if (!err) {
                return _response.apiSuccess(res, result.length+" "+responsemsg.found , result , {page: parseInt(page) , limit: parseInt(limit),totalDocs: totalDocs })
...
function update(req ,res ){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `checkout` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {

                db.query("UPDATE checkout SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `checkout` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!result.length){
                ...
                db.query("DELETE FROM `checkout` WHERE id='" + req.params.id + "'", (err, result) => {
                    if (!err) {
